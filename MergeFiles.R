...
full_data <- join_all(list_dfs) %>% 
  select(seqn, wtint2yr, wtmec2yr, sdmvpsu, sdmvstra, riagendr, ridageyr, 
         bmdbmic, whq030m, whq500, paq744, paq746, paq750) %>% 
  filter(ridageyr %in% c(12:15)) %>% 
  dplyr::rename(`id` = seqn, `int_wt` = wtint2yr, `exam_wt` = wtmec2yr, `psu` = sdmvpsu, 
                `stratum` = sdmvstra, `gender` = riagendr, `age` = ridageyr, `bmi` = bmdbmic, 
...
write.csv(full_data, "~/Desktop/Stat152/FinalProject/full_data_v3.csv")
